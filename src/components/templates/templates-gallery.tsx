...
export function TemplatesGallery() {
  ...
  return (
    <div className="flex flex-col gap-10">
      ...
      >
        <button
          ...
          className={cn(
            ...
            lane === ALL
              ? "border-cyan-400/35 bg-cyan-500/15 text-cyan-100 shadow-[0_0_24px_rgba(56,200,255,0.12)]"
        ...
        {lanes.map((l) => (
          <button
            ...
            className={cn(
              "rounded-full border px-4 py-2 text-xs font-semibold uppercase tracking-wider transition",
              lane === l
                ? "border-violet-400/35 bg-violet-500/12 text-violet-100 shadow-[0_0_22px_rgba(167,139,250,0.12)]"
                : "border-white/[0.08] bg-white/[0.03] text-zinc-500 hover:border-white/[0.12] hover:text-zinc-300",
            )}
          >
            {l}
          ...
          >
            ...
            >
              <Link
                ...
                className="absolute inset-0 z-0 rounded-3xl outline-none ring-cyan-400/0 transition focus-visible:ring-2 focus-visible:ring-cyan-400/35"
              ...
              <div className="pointer-events-none relative z-10 flex flex-1 flex-col p-6 sm:p-7">
                <div className="flex flex-wrap items-center gap-2">
                  ...
                  {typeof t.estMinutes === "number" ? (
                    <span className="inline-flex items-center gap-1 rounded-full border border-white/[0.06] bg-black/25 px-2.5 py-1 text-[10px] font-medium uppercase tracking-wider text-zinc-500">
                      <Clock className="size-3 opacity-70" aria-hidden />
                      {t.estMinutes} min
                    </span>
                  ) : null}
                </div>
                <h2 className="mt-4 font-[family-name:var(--font-display)] text-xl font-semibold leading-snug tracking-tight text-zinc-50 group-hover:text-white sm:text-[1.35rem]">
                  {t.title}
                </h2>
                ...
                <div className="mt-5 flex flex-wrap gap-2">
                  {t.tags.slice(0, 4).map((tag) => (
                    <span
                      key={tag}
                      className="rounded-full border border-white/[0.06] bg-white/[0.03] px-2.5 py-1 text-[10px] font-medium uppercase tracking-wider text-zinc-500"
                    >
                      {tag}
                    </span>
                  ))}
                </div>
                <div className="relative z-20 mt-6 flex items-center justify-between gap-3 border-t border-white/[0.06] pt-5">
                  ...
                  <Link
                    href={`/templates/${t.id}`}
                    className="pointer-events-auto text-[11px] font-medium uppercase tracking-[0.18em] text-zinc-500 transition hover:text-zinc-300"
                  >
                    Lane brief
                  </Link>
                </div>
      ...
      {filtered.length === 0 ? (
        <p className="text-sm text-zinc-500">No templates in this lane.</p>
      ) : null}
    </div>
  );
}
