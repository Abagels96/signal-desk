  ...
  return (
    <div className="flex flex-col gap-10 lg:gap-12">
      <nav className="text-sm text-zinc-500">
        <Link href="/templates" className="text-cyan-300/80 hover:text-cyan-200">
          Templates
        </Link>
        <span className="mx-2 text-zinc-600">/</span>
        <span className="text-zinc-400">{template.lane}</span>
      </nav>
        ...
        <div className="max-w-3xl">
          <div className="flex flex-wrap items-center gap-2">
            ...
            {typeof template.estMinutes === "number" ? (
              <span className="inline-flex items-center gap-1.5 rounded-full border border-white/[0.08] bg-white/[0.04] px-3 py-1 text-[11px] font-medium uppercase tracking-wider text-zinc-400">
                <Clock className="size-3.5 opacity-70" aria-hidden />
                ~{template.estMinutes} min to first draft
              </span>
            ) : null}
          </div>
          <h1 className="mt-5 font-[family-name:var(--font-display)] text-3xl font-normal tracking-tight text-zinc-50 sm:text-4xl lg:text-[2.35rem]">
            {template.title}
          </h1>
          ...
          <div className="mt-8 flex flex-wrap gap-3">
            <Link href={generateHref} className={buttonClasses("primary", "lg")}>
              Open in Generate
      ...
      <div className="grid gap-6 lg:grid-cols-12 lg:gap-8">
        ...
        <div className="flex flex-col gap-4 lg:col-span-5">
          <div className="rounded-3xl border border-white/[0.07] bg-[linear-gradient(160deg,rgba(18,22,32,0.85)_0%,rgba(10,12,18,0.94)_100%)] p-6 shadow-[inset_0_1px_0_rgba(255,255,255,0.04)]">
